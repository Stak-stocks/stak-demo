...
import { WATCH_TICKERS } from "./recommendations.js";

/**
 * A saved company's risk snapshot, generated by the daily pass. Shares the service's own
 * cache, so a stock warmed here costs the page that opens it nothing.
 */
async function warmRiskWatch(ticker: string, companyName: string): Promise<void> {
	...
	const peers = await cacheGet<{ pe: number | null }>(`peer-metrics:v2:${ticker.toUpperCase()}`)
		?? await cacheGet<{ pe: number | null }>(`peer-metrics:v2:${symbol}`);
	const margin = m.netProfitMarginTTM != null && Math.abs(m.netProfitMarginTTM) <= 500 ? m.netProfitMarginTTM : null;
	await getRiskWatch({
		symbol,
		companyName,
		...
		profitMargin: margin != null ? `${margin.toFixed(1)}%` : null,
		beta: m.beta ?? null,
	});
}
...
// Today's biggest movers (either direction) over the deck's watch universe -
// Home's Trending strip. Same quote:fb:{symbol} cache as batch-quotes, so this
// costs nothing beyond what the deck and saved-stock views already keep warm;
// the ranked top-N itself is cached briefly so a burst of Home opens doesn't
// re-sort the whole watchlist on every request.
const TRENDING_TTL_MS = 3 * 60 * 1000;
const TRENDING_LIMIT = 8;

stockRouter.get("/trending", async (req, res) => {
	const cacheKey = "trending:v1";
	const cached = await cacheGet<unknown>(cacheKey);
	if (cached) { res.json(cached); return; }

	try {
		const results = await mapWithLimit(WATCH_TICKERS, 8, async (symbol) => {
			const fbKey = `quote:fb:${symbol}`;
			let raw = await cacheGet<Record<string, number>>(fbKey);
			if (!raw) {
				raw = (await finnhubGet(`/quote?symbol=${symbol}`)) as Record<string, number> | null;
			...
			if (!raw || typeof raw.c !== "number" || raw.c <= 0) return null;
			const brand = brands.find((b) => b.ticker.toUpperCase() === symbol);
			return {
				ticker: symbol,
				name: brand?.name ?? symbol,
				price: Math.round(raw.c * 100) / 100,
				change: Math.round((raw.d ?? 0) * 100) / 100,
				changePercent: Math.round((raw.dp ?? 0) * 100) / 100,
			};
		...
		// An incomplete pull (a bad batch of quotes) isn't worth caching over the
		// next request's chance at a fuller list — only a decent yield is kept.
		if (trending.length >= TRENDING_LIMIT / 2) await cacheSet(cacheKey, payload, TRENDING_TTL_MS);
		res.json(payload);
	} catch (e) {
		console.error("[stock] trending error:", e);
	...
	try {
		...
		for (const { ticker, name } of stocks) {
			...
			try {
				...
				// And its risk snapshot, so a saved company's is generated by the daily pass
				// rather than by whoever opens the page first.
				if (Date.now() - started > WARM_BUDGET_MS) { pending++; continue; }
				await warmRiskWatch(ticker, name).catch(() => {});
			} catch (e) {
...
stockRouter.get("/:symbol/risk-watch", async (req, res) => {
	...
	try {
		...
		// Written by /peer-metrics under the ticker as asked for, which is not always the
		// symbol the market data uses (MC.PA -> LVMUY), so both are tried.
		const raw = req.params.symbol.toUpperCase();
		const peers = await cacheGet<{ pe: number | null }>(`peer-metrics:v2:${raw}`)
			?? await cacheGet<{ pe: number | null }>(`peer-metrics:v2:${symbol}`);
		// A pre-revenue company can report -10000%, which means nothing to anyone.
		const margin = m.netProfitMarginTTM != null && Math.abs(m.netProfitMarginTTM) <= 500 ? m.netProfitMarginTTM : null;
		const result = await getRiskWatch({
			symbol,
			companyName: brand?.name ?? symbol,
			// The peer median is built from peNormalizedAnnual, so this side uses it too -
			// a trailing P/E against a normalised median is a rating from a definition
			// mismatch rather than from the market.
			peRatio: m.peNormalizedAnnual ?? m.peTTM ?? null,
			peerPe: peers?.pe ?? null,
			revenueGrowth: m.revenueGrowthTTMYoy != null ? `${m.revenueGrowthTTMYoy.toFixed(1)}%` : null,
			profitMargin: margin != null ? `${margin.toFixed(1)}%` : null,
