			...
			{vibes.map((vibe, i) => (
				<div key={vibe.name} className="space-y-1.5">
					...
					<div className="relative" ref={openPopover === i ? popoverRef : undefined}>
						<button
							className="flex items-center gap-1.5 text-sm font-bold text-foreground cursor-pointer hover:text-zinc-200 transition-colors active:scale-95 touch-manipulation"
							type="button"
						...
						{openPopover === i && (
							<div
								className="absolute left-0 bottom-full mb-2 z-50 w-64 sm:w-72 rounded-xl border border-slate-700/50 px-4 py-3 text-sm text-foreground/95 leading-relaxed shadow-xl"
								style={{
